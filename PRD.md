...
- **作为**:任何公司内部用户
- **我想**:点击登录页的"通过 W3 统一认证登录"按钮
- **以便**:用公司统一身份进入招聘助手
- **触发场景**:登录页,用户未登录
- **AC**:
  - Given 我在登录页,When 点击"通过 W3 统一认证登录",Then 系统调用 W3 认证
...
- **我想**:登录后默认进入"简历解析"页,并看到当前无业务权限的状态
- **以便**:知道需要联系管理员开通业务权限,但入口保持与其他用户一致
...
- **AC**:
  ...
  - And 登录后跳转到默认主页"简历解析"
  - And 简历解析页内容展示游客空状态:欢迎语、本人工号、说明文字"您当前为游客身份,请联系管理员分配业务角色"
  - And 仅可 List Department / Get self User
  - And 简历解析页的简历选择、岗位选择、开始解析按钮不可用;推荐 / 简历库 / 配置 / 用户管理 / 角色管理 业务菜单隐藏或置灰
- **边界用例**:
  - 超级管理员或授权管理角色后续给该用户 Create UserDepartmentRole(具体部门, 业务角色):用户重新登录后,默认主页切换,业务菜单可用
  - 游客尝试直接访问业务 URL(改 URL):后端鉴权拒绝,前端引导回"简历解析"页的游客空状态
...
| 通知列表为空 | "暂无新的推荐提醒" |
| 简历解析页游客空状态 | "您当前为游客身份,请联系管理员分配业务角色" |
...
|------|:----------------:|:----------------:|--------------|
| W3 首次登录(本系统无记录) | **Create** | 无 | 进入简历解析页游客空状态;仅 Department.List + User.Get(self) |
...
| E2E 测试 | 登录、游客空状态、解析、推荐、简历库搜索、角色分配、通知跳转 |
