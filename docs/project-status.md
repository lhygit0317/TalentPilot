...
| Frontend | Component rules documented | Done | Component convention is documented in `AGENTS.md` and SPEC; `apps/web/scripts/check-business-ui-elements.mjs` enforces raw interactive element usage through `make lint`. | Keep checker aligned with future UI wrapper decisions. |
...
| CI | Quality gate workflow | Done | `.github/workflows/ci.yml` runs pnpm setup, Go setup, PostgreSQL service-backed tests, lint, typecheck, OpenAPI/client drift checks, build, installs Playwright Chromium, and runs `make test-e2e` after build. | Keep CI aligned with Makefile. |
...
| E4 | Resume library | Done | `docs/specs/003-resume-library-import.md` implemented: permission-filtered list/detail/delete, import jobs, audit coverage, API routes, generated OpenAPI/client wrappers, and frontend list/search/detail/import/delete workflows. Verification at E4 completion: `PATH=/Users/lhy/.cache/codex-go/go1.26.4-darwin-arm64/go/bin:$PATH make test-api`, `CI=true pnpm --filter @talentpilot/web test -- --run`, `make typecheck`, `PATH=/Users/lhy/.cache/codex-go/go1.26.4-darwin-arm64/go/bin:$PATH make lint`, `PATH=/Users/lhy/.cache/codex-go/go1.26.4-darwin-arm64/go/bin:$PATH make build`. Playwright smoke coverage was later enabled in quality hardening. | Use E4 as dependency for E5/E2/E3/E7. |
| E5 | Department and position management | Done | `docs/specs/004-position-department-management.md` implemented: permission-filtered department and position list/detail, super-admin writes, delete protection, audit coverage, API routes, generated OpenAPI/client wrappers, and frontend management workflows. Verification at E5 completion: `PATH=/Users/lhy/.cache/codex-go/go1.26.4-darwin-arm64/go/bin:$PATH CI=true make ci`, `git diff --check`. Playwright smoke coverage was later enabled in quality hardening. | Use E5 as dependency for E2/E3/E6. |
...
| Area | Item | Status | Evidence | Next |
| --- | --- | --- | --- | --- |
| Frontend | Component rule enforcement | Done | Task 3 implemented `apps/web/scripts/check-business-ui-elements.mjs`, `apps/web/scripts/check-business-ui-elements.test.mjs`, and `apps/web/src/components/ui/table.tsx`; web lint runs the checker. Verification: `pnpm --filter @talentpilot/web lint`, `CI=true pnpm --filter @talentpilot/web test -- --run`. | Keep enforced as business pages expand. |
| Testing | Playwright E2E smoke coverage | Done | `apps/web/playwright.config.ts` and `apps/web/e2e/*.spec.ts` provide smoke coverage; `make test-e2e` runs it. GitHub Actions installs Chromium with `pnpm --filter @talentpilot/web exec playwright install --with-deps chromium` before the E2E step. | Keep smoke coverage focused on high-risk workflows. |

## Current Recommended Order

1. Choose the next product phase or quality hardening target.
2. Keep future notification expansion available unless product priority changes.
